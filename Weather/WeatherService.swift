...
struct NetworkConstant {
    static var url: String = "https://api.openweathermap.org/data/2"
}
...
public final class WeatherService: NSObject {
  ...
  private func makeDataRequest(forCoordinates coordinates: CLLocationCoordinate2D) {
    // need url builder
    guard let urlString =
            NetworkConstant.url + "weather?lat=\(coordinates.latitude)&lon=\(coordinates.longitude)&appid=\(API_KEY)&units=metric"
...
// Hranenie danyh vmeste s service
